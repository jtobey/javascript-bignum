...
#if 0
static bool
call_mpz_add (NPObject *npobj,
              const NPVariant *args, uint32_t argCount, NPVariant *result)
{
  bool ok = true;
  if (argCount == 3 && isInteger (&args[0]) && isInteger (&args[1])) {
    if (isInteger (&args[2]))
      mpz_add (toInteger (&args[0])->mpz, toInteger (&args[1])->mpz,
               toInteger (&args[2])->mpz);
    else if (isFiniteNative (&args[2])) {
      double arg2 = toDouble (&args[2]);
      unsigned long int ui = arg2;
      if (arg2 == ui)
        mpz_add_ui (toInteger (&args[0])->mpz, toInteger (&args[1])->mpz, ui);
      else
        ok = false;
    }
    else
