...
function implementExactDecimal(plugins) {
    ...
    function ExactDecimal_toInteger() {
        var ret = toE(this, 0);
        if (ret === _undefined)
            raise("&assertion", "not an integer", this);
        return ret;
    }
    ...
    function getSameExponent(ed1, ed2) {
        ...
        return [toE(ed1, ed2._e), ed2._ns[0], ed1._e];
    }
    ...
    function ExactDecimal_exp10(e) {
        var exactE = add(e, nativeToExactInteger(this._e));
        var newE = e + this._e;
        if (eq(exactE, nativeToExactInteger(newE)))
            return new ExactDecimal(this._ns, newE, this._lim + e);
    ...
    function install() {
        ...
        plugins.get("canonicalExactInteger").def(
            ExactDecimal, ExactDecimal_toInteger);
        ...
        def("exp10", ExactDecimal, EI, ExactDecimal_exp10);
    }
